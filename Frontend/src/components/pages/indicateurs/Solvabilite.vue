...
const updateInterpretationCards = () => {
  console.log('Updating carousel data:', {
    AutonomieFinanciere: AutonomieFinanciere.value,
    CapaciteRemboursement: CapaciteRemboursement.value,
    RatioEndettement: RatioEndettement.value,
  ...
  interpretationCardsData.value = [
    {
      ...
      variation: comparisons.value?.Autonomie?.percentage || '0',
      colorVariation: getTrendClass(comparisons.value?.Autonomie),
      interpretation: AutonomieFinanciere.value?.autonomie_financiere?.interpretation || "Évolution des revenus totaux",
      format: 'percentage'
    },
    {
      texte: "Capacité de remboursement",
      chiffre: formatMoney(comparisons.value.Remboursement.evolution),
      ...
      interpretation: CapaciteRemboursement.value?.capacite_remboursement?.interpretation || "Évolution des dépenses totales",
      format: 'number'
    },
    {
      ...
      variation: comparisons.value?.Endettement?.percentage || '0',
      colorVariation: getTrendClass(comparisons.value?.Endettement),
      interpretation: RatioEndettement.value?.ratio_endettement?.interpretation || "Évolution du résultat net",
      format: 'percentage',
      negative: true
    }
    // ,
...
watch([() => AutonomieFinanciere.value, () => CapaciteRemboursement.value, () => RatioEndettement.value, () => comparisons.value], () => {
  if (!loading.value) {
...
<template>
  <PageAnalyse :menu="'Indicateurs & ratios'" :sousmenu="'Indicateurs de solvabilité'">
